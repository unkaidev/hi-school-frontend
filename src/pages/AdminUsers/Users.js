...
import "./users.scss"
import { fetchUsersWithManagerRole, deleteUser } from "../../services/userServices";
...
const Users = (props) => {
    ...
    const checkUserRole = () => {
        ...
        console.log(roles)
        if (username && roles && roles.includes('ROLE_ADMIN')) {
            setIsAdmin(true);
        }
    ...
    const fetchUsers = async () => {
        let response = await fetchUsersWithManagerRole(currentPage, currentLimit, username);
                            ...
                            <div className="user-header row">
                                <div className="title mt-3">
                                    {isAdmin ?
                                        ...
                                        :
                                        <h3>Danh sách tài khoản quản lý</h3>
                                    }
                                </div>
                                                        ...
                                                        <tr key={`row-${index}`}>
                                                            ...
                                                            <td>
                                                                {item.roles && item.roles.length > 0 ? (
                                                                    <span>
                                                                        {item.roles.map(role => (
                                                                            <span key={role.id}>{role.name.substring(5)}</span>
                                                                        ))}
                                                                    </span>
                                                                ) : ''}
                                                            </td>
